...
fn main() {
    ...
    for (i, row) in file_lines.iter().enumerate(){
        for (j, col) in row.chars().enumerate(){
            if col != '.'{
                galaxies.push((i,j,col.to_string()));
            }
        }
    }
    ...
    println!("{:?}",galaxies);
    let mut optimal_path_steps: Vec<usize> = vec![];
    ...
    println!("the sum of optimal steps is: {}", optimal_path_steps.iter().sum::<usize>());
}
