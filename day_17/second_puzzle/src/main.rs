...
fn get_neighbours(current_node_head: &(isize, isize, String, isize, isize),
    ...
    if current_node_head.3 < 3 && current_node_head.2 != ""{
        return neighbours_raw.iter()
        .filter(|node| (node.0 < matrix.len() as isize && node.0 >= 0) && (node.1 < matrix[(matrix.len()-1) as usize].len() as isize && node.1 >= 0))
        .filter(|node| node.2 == current_node_head.2)
        .map(|node| 
            (node.0, 
            node.1, 
            node.2.to_string(), 
            current_node_head.3 + 1,
            current_node_head.4 + matrix[node.0 as usize][node.1 as usize] as isize))
        .filter(|node| ((node.0 as usize, node.1 as usize) != (matrix.len()-1, matrix[matrix.len()-1].len()-1)) || ((node.0 as usize, node.1 as usize) == (matrix.len()-1, matrix[matrix.len()-1].len()-1) && node.3 == 3))
        ...
        .map(|node| 
            ...
            current_node_head.4 + matrix[node.0 as usize][node.1 as usize] as isize))
        .filter(|node| node.3 < 10 && node.2 != get_opposite_direction(&current_node_head.2))
        .filter(|node| ((node.0 as usize, node.1 as usize) != (matrix.len()-1, matrix[matrix.len()-1].len()-1)) || ((node.0 as usize, node.1 as usize) == (matrix.len()-1, matrix[matrix.len()-1].len()-1) && node.3 >= 3))
        .collect::<Vec<(isize, isize, String, isize, isize)>>()
}  
...
fn main() {
    ...
    while !current_queue.peek().is_none(){
        ...
        if (current_node_head.0 as usize, current_node_head.1 as usize) == (heat_matrix.len()-1, heat_matrix[heat_matrix.len()-1].len()-1){
            println!("end head {:?}", current_node_head);
            break;
        }
        ...
        // println!("current head {:?}", current_node_head);
