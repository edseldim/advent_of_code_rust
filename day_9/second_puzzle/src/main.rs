...
fn main() {
    ...
    for (i, report_sequence) in environment_sequences.iter_mut().enumerate(){
        ...
        for seq_num in 0..reversed_seq.len(){
            if seq_num >= 1{
                let prev_below_level_value: i32 = reversed_seq[seq_num-1][0];
                let prev_same_level_value: i32 = reversed_seq[seq_num][0];
                let same_level_pred_value:i32 = prev_same_level_value - prev_below_level_value;
                reversed_seq[seq_num].insert(0,same_level_pred_value);
            }
            
        }
    }
    println!("modified report sequences {:?}", environment_sequences);
    let pred_sum: i64 = environment_sequences.iter().map(|x| *x.sequences.first().unwrap().first().unwrap() as i64).sum();
